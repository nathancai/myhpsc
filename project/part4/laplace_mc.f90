...
program laplace_mc
    ...
    integer :: i, i0, j0, ierr, max_steps, n_mc, n_success, n_total, nwalks_total, seed1, proc_num, numprocs
    ...
    call MPI_REDUCE(nwalks, nwalks_total, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    
    if (proc_num == 0) then
    print '("Final approximation to u(x0,y0) : ", es22.14)', u_mc_total
    print *, "Total number of random walks: ", nwalks_total
    endif
    
    call MPI_BARRIER(MPI_COMM_WORLD,ierr)
    
    print '("Walks performed by Process ", i2, ": ", i8)', proc_num, nwalks
